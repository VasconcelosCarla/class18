...
function setup(){
  ...
  fimDeJogo = createSprite(width/2,height/2 - 70);
  fimDeJogo.addImage("fimDeJogo",ImgFimDeJogo);
  fimDeJogo.scale = 0.8;
  fimDeJogo.visible = false;
  
  reiniciar = createSprite(width/2,height/2 - 10);
...
function gerarNuvens(){
  ...
  if(frameCount % 60 === 0){
    ...
    nuvem.y = Math.round(random(100,height/2 - 100));
...
function gerarObstaculos(){
  if(frameCount % 60 === 0){
    var obstaculo = createSprite(width,height - 90,10,40);
